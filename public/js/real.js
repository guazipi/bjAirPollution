...
(function () {
    ...
    var OVERLAY_TYPES = {
        "temp": {min: -10, max: 35, scale: "line", precision: 1, label: "气温 Temperature", unit: "ºC"},
        "hum": {min: 0, max: 100, scale: "line", precision: 1, label: "湿度 Humidity", unit: "%"},
        "wv": {min: 1, max: 20, scale: "log", precision: 1, label: "风速 Wind Velocity", unit: " m/s"},
        "in": {
            min: 0.1,
            max: 4.0,
            scale: "log",
        ...
        "O3": {
            ...
            scale: "log",
            precision: 0,
            label: "臭氧 Photochemical Oxidants",
            unit: ' μg/m<span class="sup">3</span>',
            multiplier: 1000
        },
        "SO2": {
        ...
        "CO": {min: 0.1, max: 3.0, scale: "log", precision: 1, label: "一氧化碳 Carbon Monoxide", unit: ' μg/m<span class="sup">3</span>'},
        "PM10": {
            min: 0.01,
            max: 1.30,
            scale: "log",
            precision: 2,
            label: "可吸入颗粒物 inhalable particles",
            unit: ' μg/m<span class="sup">3</span>'
        },
        "PM2.5": {
            min: 1,
            ...
            scale: "log",
            precision: 0,
            label: "细颗粒物 2.5µm Particulate Matter",
            unit: ' μg/m<span class="sup">3</span>'
        }
    };
    ...
    function init() {
        ...
        if ("ontouchstart" in document.documentElement) {
            ...
            }, false);  // this hack enables :active pseudoclass
        }
        else {
    ...
    function buildMeshes(topo, settings) {
        ...
        var outerBoundary = topojson.mesh(topo, topo.objects.bjquan, function (a, b) {
            return a === b;
        });
        var divisionBoundaries = topojson.mesh(topo, topo.objects.bjquan, function (a, b) {
            return a !== b;
        });
    ...
    function plotStations(data, mesh) {
        ...
        //    }
        //});
        data.stations.forEach(function (e) {
            features.push({
                type: "Features",
                properties: {name: e.Name.toString()},
    ...
    function buildPointsFromSamples(stations, samples, projection, transform) {
        ...
        samples.forEach(function (sample) {
            if (sample.Pollutant == displayData.type) {
                stations.forEach(function (station) {
                    if (sample.Station == station.Name) {
                        var point = projection([station.Lon, station.Lat]);
    ...
    function buildPointsFromGFS(projection) {
        var points = [];
        ...
        $.getJSON("dataset/weather/current/realTimeWind.json", function (data) {
            ...
            for (var la1 = 39.0; la1 <= 42.0; la1 += 0.25) {
                for (var lo1 = 115.0; lo1 <= 118.0; lo1 += 0.25) {
                    var point = projection([lo1, la1]);
                    if (firstObj.data !== [] && secondObj.data !== []) {
                        points.push([point[0], point[1], [firstObj.data[0], secondObj.data[0]]]);
                        firstObj.data.shift();
    ...
    function interpolateField(data,settings, masks) {
        ...
        //});
        var points = buildPointsFromGFS(settings.projection);

        if (points.length < 5) {
            return d.reject("北京市环境保护局正在调整中");
        }
    ...
    function drawOverlay(stations, data, settings, masks) {
        ...
        //});
        var points = buildPointsFromSamples(stations.stations, data.Table, settings.projection, function (sample) {
            //var datum = sample[displayData.type];
            //return datum == +datum ? datum : null;


            var datum = sample.Value;
            return datum == +datum ? datum : null;
    ...
    //init();
    ////var settingsTask = createSettings(topoTask);
    //var settingsTask     = when.all([topoTask                             ]).then(apply(createSettings));
    //var meshTask = buildMeshes(topoTask, settingsTask);
    //var renderTask = render(settingsTask, meshTask);
    //var plotStationTask = plotStations(stations, meshTask);
    //var overlayTask=drawOverlay(stations,webdata,settingsTask,renderTask);
